...
try {
  ...
  if ($action === "get_coordinator_courses") {

    $data = json_decode(file_get_contents("php://input"), true);

    $coordinatorId = (int)($data["coordinator_id"] ?? 0);
    ...
    $courses = array_column($stmt->fetchAll(), "course_id");
  ...
  if ($action === "set_coordinator_courses") {
    ...
    $stmt = $pdo->prepare("
      INSERT INTO coordinator_courses (coordinator_id, course_id)
      VALUES (?, ?)
    ");

    foreach ($courseIds as $cid) {
      $stmt->execute([$coordinatorId, (int)$cid]);
    }
